...
void random_walk_generation(int initx, int inity){
	
	// escojer punto de partida
	int curr_row = initx, curr_column = inity; //Partir del centro
...
int main(int argc, char const *argv[])
{
	...
	/*#pragma omp parallel
	{
		#pragma omp sections
		{
			#pragma omp section
			{
				random_walk_generation(N/2+directions[0][0],N/2+directions[0][1]);	
			}
			#pragma omp section
			{
				random_walk_generation(N/2+directions[1][0],N/2+directions[1][1]);
			}
			#pragma omp section
			{
				random_walk_generation(N/2+directions[2][0],N/2+directions[2][1]);
			}
			#pragma omp section
			{
				random_walk_generation(N/2+directions[3][0],N/2+directions[3][1]);
			}
		}
	}*/
	...
	#pragma omp parallel
	{
		...
		{
//			cout << omp_get_thread_num()<<endl;
			random_walk_generation(N/2+directions[i][0],N/2+directions[i][1]);
		}
	}
	...
	#pragma omp parallel
	{
		#pragma omp for
			for (int i = 0; i < 16; ++i)
			{
				applyGameOfLife(inicio[i][0],inicio[i][1],final[i][0],final[i][1]);
			}
	}
